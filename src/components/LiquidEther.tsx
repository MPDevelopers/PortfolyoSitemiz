  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      init(container: HTMLElement) {
        ...
        const ctxOpts = { alpha: true, antialias: true, premultipliedAlpha: true, powerPreference: 'high-performance' } as const;
        const gl =
        ...
        this.renderer = new THREE.WebGLRenderer({ canvas, context: gl as any, antialias: true, alpha: true });
...
`;
    ...
    void main(){
    vec2 vel = texture2D(velocity, uv).xy;
    float lenv = clamp(length(vel) * 4.0, 0.0, 1.0);
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props: any) {
        ...
        const uniforms = (this.mouse as any).material.uniforms;
    ...
    class Simulation {
      ...
      createAllFBO() {
        ...
        const supportsLinear = !!(Common.renderer && ((Common.renderer as any).capabilities?.isWebGL2 || (Common.renderer as any).extensions?.has?.('OES_texture_float_linear')));
        const minMag = supportsLinear ? THREE.LinearFilter : THREE.NearestFilter;
        const opts = {
          type,
          depthBuffer: false,
          stencilBuffer: false,
        ...
        } as THREE.WebGLRenderTargetOptions;
